...
async function getTotalPsPacketById(psId) {
  try {
    const [RstClientHttp, RstServerHTTP,RstClientTls, RstServerTls, totalRxHttpCount,totalRxTlsCount] = await Promise.all([
      PsPacket.sum('rstClient_http', { where: { ps_id: psId } }),
      PsPacket.sum('rstServer_http', { where: { ps_id: psId } }),
      PsPacket.sum('rstServer_tls', { where: { ps_id: psId } }),
      PsPacket.sum('rstServer_tls', { where: { ps_id: psId } }),
    ...
    return {
      psPackets: {
        rstClientHttp: RstClientHttp || 0,
        rstServerHttp: RstServerHTTP || 0,
        rstClientTls: RstClientTls || 0,
        rstServerTls: RstServerTls || 0,
